...
export function EnvironmentWhmPage() {
  ...
  const whmConnections = (connectionsQuery.data ?? []).filter(
    (c) => c.kind === "whm",
  );
  ...
  const whmCredentials = (credentialsQuery.data ?? []).filter(
    (c) => c.kind === "whm_api_token",
  );
  ...
  const addConnectionMutation = useMutation({
    mutationFn: (input: {
      ...
      addWhmConnection(environmentId, input.host, input.port, input.credential),
    onSuccess: () => {
      showSuccess("WHM connection added");
    ...
    onError: (err) =>
      showError(`Failed to add WHM connection: ${describeError(err)}`),
  });
  ...
  return (
    <form
      onSubmit={(e) => {
        ...
        const credential: WhmCredentialInput =
          credentialMode === "existing"
